...
class Wechat {
	 ...
	 */
	private $wechatOptions=array('token'=>'','account'=>'','password'=>'');	//
	...
	public function login($code=''){
		...
		$postfields["f"] = "json";
		//有验证码的时候
		if(!empty($code)) $postfields["imgcode"] = $code;
		//$postfieldss = "username=".urlencode($this->wechatOptions['account'])."&pwd=".urlencode(md5($this->wechatOptions['password']))."&f=json&imgcode={$code}";
		$this->curlInit("single");
		//有验证码的时候
		if(!empty($code))
		{
			$m_temp = file_get_contents(WEB_ROOT.'/apply/memberSetting/temp_cookies.txt');
			$m_postion = stripos($m_temp,'sig');
			$m_str = substr($m_temp,$m_postion+3);
			$m_str = trim($m_str);
			$m_cookie['sig'] = $m_str;
			$response = $this->_curlHttpObject->post($url, $postfields, $this->protocol."://mp.weixin.qq.com/cgi-bin/login",$m_cookie);		
		}
		else
		{
	 ...
	 **/
	 private function doQueueSend($queueSendArray, $queueCount=10)
	 {
		...
		foreach ($queueSendArray as $key =>$value)
		{
			...
			$postfields['ajax'] = 1;
			$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/singlesend?t=ajax-response";
			$requestArray[] = array('url'=>$url,'method'=>'post','postfields'=>$postfields,'referer'=>$this->protocol."://mp.weixin.qq.com/cgi-bin/singlemsgpage?",'cookies'=>$this->_cookies);
		}
...
// 					file_put_contents("log.txt", "False\n",FILE_APPEND);
	...
	{
		$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize={$pagesize}&pageidx=0&type=0&groupid={$groupid}&token={$this->webtoken}&lang=zh_CN";
		...
		$start = stripos($response,'friendsList');
		if($start)
		{
			$end = stripos($response,'.contacts');
			$str = substr($response,$start+15,$end-16-$start);
			$str = json_decode($str, true);
			$str = $str['contacts'];
			foreach($str as $k=>$v)
			{
				$nv['fakeId'] = $v['id'];
				$nv['nickName'] = $v['nick_name'];
				$nv['remarkName'] = $v['remark_name'];
				$nv['groupId'] = $v['group_id'];
				$tmp[] = $nv;
			}
		}
	...
	/**
	 * Add By mflaugh Time:2013-08-12
	 * 获取用户的头像信息
	 * @param  string $fakeid 用户的fakeid
	 * @param  string $usrname 用户所属的公众平台用户名
	 * @param  string $fliename 头像保存文件名
	 * @return mixed 如果成功返回本地保存图片路径,失败返回false.
	 */
	public function getContactImg($fakeid,$usrname,$fliename)
	{
		$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/getheadimg?token={$this->webtoken}&fakeid=$fakeid";
		$this->curlInit("single");
		$response = $this->_curlHttpObject->get($url,$this->protocol."://mp.weixin.qq.com/cgi-bin/",$this->_cookies);
		if(!file_exists(WEB_ROOT.'/files/'.$usrname))
		{
			mkdir(WEB_ROOT.'/files/'.$usrname);
		}
		if(!file_exists(WEB_ROOT.'/files/'.$usrname.'/friends/'))
		{
			mkdir(WEB_ROOT.'/files/'.$usrname.'/friends/');
		}
		$filepath = WEB_ROOT.'/files/'.$usrname.'/friends/'.$fliename.'.jpg';
		file_put_contents($filepath,$response);
		if(file_exists($filepath))
		{
			return WEB_PATH.'files/'.$usrname.'/friends/'.$fliename.'.jpg';
		}
		else
		{
			return false;
	...
	/**
	 * Add By mflaugh Time:2013-08-14
	 * 上传素材,若成功 返回 素材ID
	 * @param  array $content     内容数组
	 * @return   若成功 返回 素材ID
	 */
	public function uploadFodder($postfields,$count)
	{
		//判断cookie是否为空，为空的话自动执行登录	
		if (file_exists($this->cookiefilepath)||true===$this->login())
		{
			...
			$tmp = json_decode($response,true);
			if ('OK'==$tmp["msg"]) {
				$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/operate_appmsg?token={$this->webtoken}&lang=zh_CN&sub=list&t=ajax-appmsgs-fileselect&type=10&subtype=3&pagesize=1&pageidx=0";
				$postfields=array();
				$response = $this->_curlHttpObject->get($url, $this->protocol."://mp.weixin.qq.com/", $this->_cookies);
				$tmp = json_decode($response,true);
				return $tmp['List'][0]['appId'];
			}
			elseif ($tmp['ret']=="-2000")
			{
				return -1;
	...
	public function uploadFile($filepath,$type=2) {
		...
		$send_snoopy->rawheaders['Cookie']= $m_cookies;
	...
	/**
	 * Add By mflaugh Time:2013-08-14
	 * 执行主动发送图文消息队列，默认并发队列数是10
	 * @param array $fakeIdArray 发送队列fakeid数组 
	 * @param array $msgId 消息ID 
	 * @param integer $queueCount 并发数量,默认10
	 * @return array  返回一个记录发送结果的数组列表
	 **/
	 public function sendNews($fakeIdArray,$msgId,$queueCount=10)
	 {
		set_time_limit(0);
		$requestArray = array();
		foreach ($fakeIdArray as $key =>$value)
		{
			$postfields = array();
			...
			$postfields['error']= "false";
			$postfields['token']= $this->webtoken;
			$postfields['fid'] = $msgId;
			$postfields['appmsgid'] = $msgId;
			$postfields['ajax'] = 1;
			$postfields['imgcode'] = '';
