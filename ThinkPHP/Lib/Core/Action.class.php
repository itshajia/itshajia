...
abstract class Action {
   ...
   /**
    ...
    protected function redirect1($url,$params=array(),$delay=0,$msg='') {
        $url    =   U($url,$params);

        // 自定义添加，只对本项目有效，非官方提供
        $url = preg_replace("/App\/([^\/]*)(\/?)(.*)\.php(\/?)(.*)/", '$5', $url);
        redirect($url,$delay,$msg);
    }

    protected function redirect2($url,$params=array(),$delay=0,$msg='') {
        $url    =   U($url,$params);

        // 自定义添加，只对本项目有效，非官方提供
        $url = preg_replace("/App\/([^\/]*)(\/?)(.*)\.php(\/?)(.*)/", '$1/$5', $url);
        redirect($url,$delay,$msg);
    }
